...
uint32_t Update_clunum_next()
{
    ...
    // char * temp1 = "have find";
    // char * temp2 = "have write";
...
{
    clunum_available =  (uint32_t)((int)clunum_available + change);
...
uint32_t create_clu()
{
    ...
    // 更新
    Update_clunum_next();
    // char * temp1 = "over_update_clunum_next";
    // println(temp1);
    Update_clunum_available(-1);
    // char * temp2 = "over_update_clunum_available";
    // println(temp2);
    return TMP_clunum_next;
}

// 向新创建的目录添加. 和 .. 的信息
uint32_t create_dirinfo(uint32_t aimclu, uint32_t nowclu)
{
    char name1[5];
    name1[0] = '.';
    name1[1] = (char)0;
    ...
    name2[1] = (char)'.';
    name2[2] = (char)0;
    char attr_sys = (char)0x42;
    char tmp_buffer[file_info_len];
    char tmp_file[file_info_len];
    bzero(tmp_file, file_info_len);
    strncpy(tmp_file + 0x00, name1, 1);
    for (int i = 1; i <= 7 + 4; i++)
    {
        tmp_file[i] = (char)0x20;
    }
    itoa_10_small(tmp_buffer, 1, (uint32_t)attr_sys);
    strncpy(tmp_file + 0x0B, tmp_buffer, 1);
    itoa_10_small(tmp_buffer, 4, aimclu);
    
    strncpy(tmp_file + 0x14, tmp_buffer + 2, 2);
    strncpy(tmp_file + 0x1A, tmp_buffer, 2);
    
    write_nbyte(ClusterToSector(aimclu), 0, file_info_len, tmp_file);
    bzero(tmp_file, file_info_len);
    strncpy(tmp_file + 0x00, name2, 2);
    for (int i = 2; i <= 7 + 4; i++)
    {
        tmp_file[i] = (char)0x20;
    }
    itoa_10_small(tmp_buffer, 1, (uint32_t)attr_sys);
    strncpy(tmp_file + 0x0B, tmp_buffer, 1);
    itoa_10_small(tmp_buffer, 4, nowclu);
    strncpy(tmp_file + 0x14, tmp_buffer + 2, 2);
    strncpy(tmp_file + 0x1A, tmp_buffer, 2);
    write_nbyte(ClusterToSector(aimclu), file_info_len, file_info_len, tmp_file);
}
// 新建文件 提供路径 名字 后缀 属性
uint32_t create_file(char *path, uint32_t pathlen, char *name, uint32_t namelen, uint8_t attr)
{
    ...
    if (((attr & 0x10) != 0) && (namepointoff != namelen))
    { // 要求是目录 且有小数点
        return ERR;
    }
    if ((namepointoff > (uint32_t)8))
    { // 长度溢出 暂时只支持短目录名 所以报错
        return ERR;
    }
    ...
    // 处理属性
    itoa_10_small(tmp_buffer, 1, (uint32_t)attr);
    strncpy(tmp_file + 0x0B, tmp_buffer, 1);
    // 非目录暂时不需要考虑文件簇号
    if (((attr & 0x10) != 0))
    { // 要求是目录
        // 获取一个新的簇号
        uint32_t new_clu = create_clu();
        create_dirinfo(new_clu, clu);
        // 文件簇号
        itoa_10_small(tmp_buffer, 4, new_clu);
        strncpy(tmp_file + 0x14, tmp_buffer + 2, 2);
        strncpy(tmp_file + 0x1A, tmp_buffer, 2);
    }
...
void change_fat_clu(uint32_t clu, uint32_t x)
{
    ...
    write_nbyte(FAT1_pos + offbrk, clu % fat_clunum * 4, 4, tmp_clunum);
}

// 清空一个簇
void del_clu(uint32_t clu)
{
    char tmp_clear[file_info_len];
    uint32_t sec = ClusterToSector(clu);
    bzero(tmp_clear , file_info_len);
    uint32_t times = (uint32_t)DBR_info.Sector_Byte / (uint32_t)file_info_len;
    for(int i = 0;i<DBR_info.Cluster_Sector;i++){
        for(int j = 0;i<times;j++){
            write_nbyte(sec+i , j*file_info_len ,file_info_len , tmp_clear);
        }
    }
    while(1);
}
